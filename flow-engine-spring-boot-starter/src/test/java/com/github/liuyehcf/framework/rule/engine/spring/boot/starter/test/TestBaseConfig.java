...
public class TestBaseConfig {
    ...
    public void testComponentAnnotation() {
        Promise<ExecutionInstance> promise = defaultFlowEngine.startRule(new ExecutionCondition("{\n" +
    ...
    public void testSpecialRule() {
        Promise<ExecutionInstance> promise = specialFlowEngine.startRule(new ExecutionCondition("{\n" +
                "    if(specialCondition()[specialListener(event=\"before\")]){\n" +
                "        specialAction()\n" +
                "    }\n" +
                "}"));
